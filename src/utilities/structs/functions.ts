...
class functions {
    ...
    public refreshAccount(accountId, username) {
    ...
    public async registerUser(discordId: any, username: string, email: string, plainPassword: string, avatarUrl: string) {
        ...
        if (plainPassword.includes(":") || email.includes(":") || username.includes(":")) return {message: "Your email, username, or password has special characters, please remove them and try again.", status: 400};
        ...
        try {
			...
			await User.create({
                ...
                await axios.get("http://127.0.0.1:3551/api/staff/setvbucks/" + Safety.env.API_KEY, {
                    headers: {
                        'accountid': i.accountId,
                        'add': "true",
                        'value': "10000"
            ...
            if (err.code == 11000) return { message: `Your username or email is already linked to an account!`, status: 400 };
            console.error(err);

            return { message: "There was an error creating your account.", status: 400 };
        };

        return { message: `Your account has been created.`, status: 200 };
    }
