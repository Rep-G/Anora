...
export const data = new SlashCommandBuilder()
    .setName('setvbucks')
    .setDescription('Set a users vbucks')
    .addStringOption(option =>
        option.setName('username')
            .setDescription('The username of the user you want to give vbucks too')
            .setRequired(true))
    .addStringOption(option =>
        option.setName('add')
            .setDescription('Whether you want to add or increase vbucks (true or false)')
            .setRequired(true))
    .addStringOption(option =>
        option.setName('value')
            .setDescription('Amount of vbucks you want too add or increase')
            .setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const response = await axios.get("http://127.0.0.1:3551/api/staff/setvbucks/" + safety.env.API_KEY, {
            headers: {
                'accountid': user.accountId,
                'add': interaction.options.getString("add"),
