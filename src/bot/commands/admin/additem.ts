...
export const data = new SlashCommandBuilder()
    .setName('additem')
    .setDescription('Add item to a users account')
    .addStringOption(option =>
        option.setName('username')
            .setDescription('The username of the user you want to add a cosmetic too')
            .setRequired(true))
    .addStringOption(option =>
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const response = await axios.get("http://127.0.0.1:3551/api/staff/additem/" + safety.env.API_KEY, {
            headers: {
                'accountid': user.accountId,
                'cosmeticname': interaction.options.getString("cosmeticname")
            }
        });
