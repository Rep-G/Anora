...
app.get("/", (req, res) => {
    const clientList = global.Clients.length > 0 
        ? global.Clients.map(i => i.displayName).join(", ") 
        : "No clients online";
    ...
    res.status(200).send(`
        ...
        <html lang="en">
            ...
            <body>
                <h1>Anora XMPP</h1>
                <h3>There are currently <strong>${global.Clients.length}</strong> online</h3>
                <h3>Clients: ${clientList}</h3>
            </body>
        </html>
    `);
});
        ...
        switch (msg.root.name) {
            ...
            case "auth":
                ...
                if (global.accessTokens) {
                    object = global.accessTokens.find(i => i.token == decodedBase64[2]);
                ...
                let user = await User.findOne({ accountId: object.accountId, banned: false, gs: false }).lean();
            ...
            case "iq":
                ...
                switch (msg.root.attributes.id) {
                    case "_xmpp_bind1":
                        ...
                        let bind = msg.root.children.find(i => i.name == "bind");
                        let findResource = bind?.children.find(i => i.name == "resource");
...
export const Clients = global.Clients;